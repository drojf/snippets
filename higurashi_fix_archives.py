...
HIGURASHIEP0X_FOLDER = 'HigurashiEp01_Data'
# !! Please set this variable to the correct Data Folder Name !!
...
def getDataFolderAndOutputNameFromArchiveName(archive_name_any_case):
	archive_name = archive_name_any_case.lower()
	if 'onikakushi' in archive_name:
		return 'HigurashiEp01_Data', 'Onikakushi-Voices'
	elif 'watanagashi' in archive_name:
		return 'HigurashiEp02_Data', 'Watanagashi-Voices'
	elif 'tatarigoroshi' in archive_name:
		return 'HigurashiEp03_Data', 'Tatarigoroshi-Voices'
	elif 'himatsubushi' in archive_name:
		return 'HigurashiEp04_Data', 'Himatsubushi-Voices'
	elif 'meakashi' in archive_name:
		return 'HigurashiEp05_Data', 'Meakashi-Voices'
	elif 'tsumihoroboshi' in archive_name:
		return 'HigurashiEp06_Data', 'Tsumihoroboshi-Voices'
	else:
		return None, None

def moveAndPrint(source_folder, destination_folder):
	print(f"Source Folder: {source_folder}")
	print(f"Destination Folder: {destination_folder}")

	if not os.path.exists(source_folder):
		raise Exception(f"Source voice folder {source_folder} doesn't exist")

	shutil.move(source_folder, destination_folder)
	
def deploy():
	fix_voice_folder = False
	...
	for filename in os.listdir():
		...
		extract_dir = 'archive_output_dir'
		staging_dir = 'staging_dir'

		try_remove_tree_repeat_attempt(extract_dir)
		try_remove_tree_repeat_attempt(staging_dir)

		# Extract the archive
		print(f'{filename}')
		extract(filename, extract_dir)

		# Create new folder structure
		HIGURASHIEP0X_FOLDER_NAME, archive_name = getDataFolderAndOutputNameFromArchiveName(filename)
		if HIGURASHIEP0X_FOLDER_NAME is None:
			raise Exception(f"Couldn't determine folder name for {filename}")

		print(f"Detected higurashi folder name as: {HIGURASHIEP0X_FOLDER_NAME}")

		# This will be the final higurashiep0x folder to be archived
		higurashiEp0x_path = os.path.join(staging_dir, HIGURASHIEP0X_FOLDER_NAME)

		# Prepare the streamingassets folder
		streaming_assets_folder = os.path.join(higurashiEp0x_path, 'StreamingAssets')
		os.makedirs(streaming_assets_folder, exist_ok=True)

		if fix_voice_folder:
			# Move the contents of the output_dir into the created folder structure
			source_folder = os.path.join(extract_dir, 'voice')
			if not os.path.exists(source_folder):
				source_folder = os.path.join(extract_dir, 'SE')

			# This is the streamingassets folder containing the renamed 'voice' folder as 'SE'
			destination_folder = os.path.join(streaming_assets_folder, 'SE')

			moveAndPrint(source_folder, destination_folder)
		else:
			for folder in os.listdir(extract_dir):
				source_folder = os.path.join(extract_dir, folder)
				destination_folder = os.path.join(streaming_assets_folder, folder)
				moveAndPrint(source_folder, destination_folder)

		# Re-archive the folder
		archive(f'./{higurashiEp0x_path}', os.path.join('updated_archives', archive_name + '.7z'))
